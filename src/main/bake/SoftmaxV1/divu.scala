...
class div (val bitwidth: Int, val bandwidth: Int) extends Module {
    ...
    val div_res_vec = RegInit(VecInit(Seq.fill(cycle_bandwidth)(0.U((frac_bitwidth).W))))
    ...
    when (state === sRun) {
        ...
        io.divu_o.bits.res_data   := VecInit(Seq.fill(cycle_bandwidth)(0.U(frac_bitwidth.W)))
        ...
        io.divu_o.valid         := false.B 
        io.divu_o.bits.res_data   := VecInit(Seq.fill(cycle_bandwidth)(0.U(frac_bitwidth.W)))
        div_res_vec             := VecInit(Seq.fill(cycle_bandwidth)(0.U(frac_bitwidth.W)))        
    }
