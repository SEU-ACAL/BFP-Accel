...
*/
    ...
    state := fsm(maxu_i_hs, maxu_shiftu_o_hs, MaxmemEmpty)

    io.maxu_i.ready        := state === sIdle
    io.maxu_shiftu_o.valid := state === sDone
    io.maxu_ldu_o.valid    := state === sDone
    // ======================================================
    val sign_mem = SyncReadMem(maxmem_depth, Vec(maxmem_width, UInt(1.W)))
    val exp_mem  = SyncReadMem(maxmem_depth, Vec(maxmem_width, UInt(exp_bitwidth.W)))
    val frac_mem = SyncReadMem(maxmem_depth, Vec(maxmem_width, UInt(frac_bitwidth.W)))

    when (maxu_i_hs) {
        for (i <- 0 until maxmem_depth) {
            for (j <- 0 until maxmem_width) {
                sign_mem(i)(j) := io.maxu_i.bits.raw_data(i*maxmem_width + j)(bitwidth - 1)              
                exp_mem(i)(j)  := io.maxu_i.bits.raw_data(i*maxmem_width + j)(bitwidth - 2, bitwidth - 6)  
                frac_mem(i)(j) := io.maxu_i.bits.raw_data(i*maxmem_width + j)(bitwidth - 7, 0)                           
            }
        }
    }
    ...
    val sign_vec     = WireInit(VecInit(Seq.fill(cycle_bandwidth)(0.U(1.W))))
    val exp_vec      = WireInit(VecInit(Seq.fill(cycle_bandwidth)(0.U((exp_bitwidth).W))))
    val frac_vec     = WireInit(VecInit(Seq.fill(cycle_bandwidth)(0.U((frac_bitwidth).W))))
    ...
    for (i <- 0 until maxmem_depth) {
        for (j <- 0 until maxmem_width) {
            datain_sign(i*maxmem_width + j) := io.maxu_i.bits.raw_data(i*maxmem_width + j)(bitwidth - 1)              
            datain_exp(i*maxmem_width + j)  := io.maxu_i.bits.raw_data(i*maxmem_width + j)(bitwidth - 2, bitwidth - 6)  
        }
    }
