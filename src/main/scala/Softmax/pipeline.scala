...
class max_shift extends Bundle { 
    ...
    val sign_vec = Vec(datain_bandwidth, UInt(1.W))
...
class sub_exp extends Bundle { val frac_vec = (Vec(datain_bandwidth, UInt(frac_bitwidth.W)))}
...
class exp_add extends Bundle { val frac_vec = Vec(datain_bandwidth, UInt(frac_bitwidth.W)) }
