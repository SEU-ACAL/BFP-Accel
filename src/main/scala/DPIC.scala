...
//         val line_num  = Input(UInt(log2Up(datain_lines).W)) 
//         // val line_size = Input(UInt(log2Up(datain_bandwidth).W)) 
//         val line_data = Output(Vec(datain_bandwidth, UInt(bitwidth.W)))
//     })
//     setInline("softmax_input_fp16_line.v",
//     """
//     |import "DPI-C" function void softmax_read_FP16_matrix(input bit en, input int line_num, input int line_size, output shortint line_data[]);
//     |
...
//     |    input en,
//     |    input [31:0] line_num,
//     |    output [15:0] line_data [LINE_SIZE-1:0]
//     |);
//     |    shortint line_data_internal [LINE_SIZE-1:0];
//     |
//     |    genvar i;
//     |    generate
//     |        for (i = 0; i < LINE_SIZE; i = i + 1) begin
//     |            assign line_data[i] = line_data_internal[i];
//     |        end
//     |    endgenerate
//     |
//     |    always @(*) begin
//     |        softmax_read_FP16_matrix(en, line_num, LINE_SIZE, line_data_internal);
//     |    end
//     |endmodule
//     """.stripMargin)
// }
...
//     |`define datain_line      1024
//     |import "DPI-C" function void softmax_read_FP16_matrix( input bit en, input int line_num, input int line_size, output svOpenArrayHandle line_data);
//     |
//     |module softmax_input_fp16_line (
//     |    input         en,
//     |    input  [31:0] line_num,
//     |    output [15:0] line_data [`datain_bandwidth-1:0] 
//     |);
//     |
//     |    // 内部数组用于存储从 C 函数接收的数据
//     |    shortint line_data_internal [`datain_bandwidth:0]; 
//     |
//     |always @(*) begin
//     |    softmax_read_FP16_matrix(en, line_num, `datain_bandwidth, line_data);
//     |    
//     |    for (int i = 0; i < `datain_bandwidth; i = i + 1) begin
//     |        line_data[i] = line_data_internal[i]; // 假设我们只处理数组的前 LINE_SIZE 个元素
//     |    end
//     |end
//     |
//     |
...
class softmax_input_fp16_line extends BlackBox with HasBlackBoxResource {
  val io = IO(new Bundle {
    val en = Input(UInt(1.W))
    val line_num = Input(UInt(log2Up(1024).W)) // 确保这个位数能够表示你的datain_lines
    val line_data = Output(Vec(1024, UInt(16.W)))
  })
  
  addResource("/softmax_input_fp16_line.v")
